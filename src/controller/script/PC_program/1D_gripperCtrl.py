...
SERVO_MAX_ANG =  PI
SERVO_MIN_ANG = -PI
SCREW_INIT_DIS = 28.02      # motor pos=0 <--> dis=122.8mm, assume that the thickness of sensor is 45mm, init_dis = 122.8-45x2=32.8mm  
...
class DgripperCtrl:
    ...
    def key_run(self):
        rospy.init_node('dgripper_ctrl', anonymous=True)
        ...
        while True:
            ...
            if now_time - self.received_time < 0.5:
                key = self.teleop[0]
                print('teleop:',key)
                self.teleop= 'n'
            else:
    ...
    def update(self):
        ...
        self.state[0] = self.board.motor[0].position * N20_RAD2DIS /1.12 
        self.state[1] = self.board.motor[1].position * N20_RAD2DIS *1.67 
        self.state[2] = (-self.board.motor[2].position * SCREW_RAD2DIS + SCREW_INIT_DIS+11.4)*1.875-25.25
