...
SERVO_MAX_ANG =  PI
SERVO_MIN_ANG = -PI
SCREW_INIT_DIS = 13      # motor pos=0 <--> dis=122.8mm, assume that the thickness of sensor is 45mm, init_dis = 122.8-45x2=32.8mm  
...
class DgripperCtrl:
    ...
    def key_run(self):
        while True:
            ...
            if now_time - self.received_time < 0.2:
                key = self.teleop[0]
                print('teleop:',key)
                self.teleop= 'n'
            else:
            ...
            # if key = contrl + c 
            if key == chr(27) or key == chr(3) or key == "q":    # ESC
                print("Quit!")
                pid = os.getpid()
            ...
            elif key == "9":        # set position of motor
            ...
            elif key == "6":        # read position of motor
                motor_id = int(input("motor id:"))
