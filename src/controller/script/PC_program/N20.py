...
N20_MAX_SPD = 4.0 #rad/s
N20_KP = 1.5
N20_KI = 0.1
N20_KD = 0.0
N20_FEED_FORWARD = 1.2
N20_I_MAX = 50
class N20Ctrl:
    def __init__(self, num):
        ...
        self.PID_ctrl = PID(N20_KP, N20_KI, N20_KD, N20_MAX_SPD, N20_FEED_FORWARD, N20_I_MAX)
    ...
    def pidLoop(self, ser):
        self.read_position(ser)
        self.PID_ctrl.cal_output(self.position)
        self.set_speed(self.PID_ctrl.output, ser)
    ...
    def read_position(self, ser):
        ...
        while not self.pos_updated:
            ...
            if count % 10 == 0 and count > 0:
                ser.write(cmd)
                print("try reread position: ", self.num)
                
            if count <= 0:
                print("read position timeout:", self.num)
                return
        self.pos_updated = False
        # print("read position successfully, position:", self.position)
